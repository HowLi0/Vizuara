...
use vizuara_plots::{LinePlot, PlotArea, ScatterPlot};
...
mod tests {
    ...
    fn test_combined_scene() {
        ...
        // 应该包含线条和点的图元
        let line_strips = primitives
            .iter()
            .filter(|p| matches!(p, vizuara_core::Primitive::LineStrip(_)))
            .count();
        let points = primitives
            .iter()
            .filter(|p| matches!(p, vizuara_core::Primitive::Points(_)))
            .count();
