...
use vizuara_plots::{ParallelCoordinates, ParallelAxis, ParallelSeries, PlotArea};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mercedes_c63 = ParallelSeries::new("Mercedes C63".to_string(), vec![510.0, 700.0, 11.5, 4.0, 95.0])
        .color(Color::rgb(0.2, 0.8, 0.2));

    let lexus_rcf = ParallelSeries::new("Lexus RC F".to_string(), vec![472.0, 530.0, 12.1, 4.4, 75.0])
        .color(Color::rgb(0.8, 0.6, 0.2));

    let tesla_model_s = ParallelSeries::new("Tesla Model S".to_string(), vec![1020.0, 1050.0, 0.0, 2.1, 120.0])
    ...
    let scene = Scene::new(plot_area)
        .add_parallel_coordinates(parallel);
