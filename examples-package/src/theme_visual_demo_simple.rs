...
use vizuara_core::{Primitive, Style, HorizontalAlign, VerticalAlign, ComponentType};
...
impl ThemeDemo {
    fn new() -> Self {
        Self {
            ...
            theme_names: vec!["Default", "Dark", "Scientific", "High Contrast"],
        }
    }
    ...
    fn apply_current_theme(&self) {
        ...
        match theme_name {
            "Default" => { let _ = manager.switch_theme("default"); }
            "Dark" => { let _ = manager.switch_theme("dark"); }
            "Scientific" => { let _ = manager.switch_theme("scientific"); }
            "High Contrast" => { let _ = manager.switch_theme("high_contrast"); }
            _ => {}
        }
    }
