...
use vizuara_core::{HorizontalAlign, Primitive, Style, VerticalAlign};
use vizuara_themes::{ComponentType, ThemeManager};
...
impl ThemeDemo {
    ...
    fn create_primitives(&self) -> Vec<Primitive> {
        ...
        let primary_color = current_theme.get_primary_color(&ComponentType::LinePlot);
        let secondary_color = current_theme.get_secondary_color(&ComponentType::BarPlot);
        ...
        // 柱状图示例
        let bar_data = [0.8, 0.6, 1.0, 0.4, 0.7];
        let bar_width = 40.0;
    ...
    fn create_styles(&self) -> Vec<Style> {
        ...
        let primary_color = current_theme.get_primary_color(&ComponentType::ScatterPlot);
        let secondary_color = current_theme.get_secondary_color(&ComponentType::LinePlot);
        let text_color = current_theme.get_text_color();
