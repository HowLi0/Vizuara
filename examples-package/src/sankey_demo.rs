...
use vizuara_plots::{PlotArea, SankeyDiagram, SankeyLink, SankeyNode};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 创建能源流向桑基图
    let mut sankey = SankeyDiagram::new().title("能源消耗流向图");
    ...
    // 添加连接
    let link1 = SankeyLink::new(
        "煤炭".to_string(),
        "发电".to_string(),
        45.0,
        Color::rgba(0.4, 0.2, 0.1, 0.6),
    );
    let link2 = SankeyLink::new(
        "石油".to_string(),
        "发电".to_string(),
        15.0,
        Color::rgba(0.2, 0.2, 0.2, 0.6),
    );
    let link3 = SankeyLink::new(
        "天然气".to_string(),
        "发电".to_string(),
        25.0,
        Color::rgba(0.3, 0.4, 0.8, 0.6),
    );
    let link4 = SankeyLink::new(
        "发电".to_string(),
        "工业".to_string(),
        40.0,
        Color::rgba(0.8, 0.4, 0.2, 0.6),
    );
    let link5 = SankeyLink::new(
        "发电".to_string(),
        "民用".to_string(),
        45.0,
        Color::rgba(0.2, 0.8, 0.2, 0.6),
    );
    ...
    let scene = Scene::new(plot_area).add_sankey_diagram(sankey);
