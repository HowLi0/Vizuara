...
use crate::{Theme, ThemePresets, ThemeResult, ThemeError};
...
impl ThemeManager {
    ...
    fn new() -> Self {
        let mut manager = Self {
            current_theme: Arc::new(RwLock::new(ThemePresets::default())),
    ...
    /// 将主题保存到文件
    pub fn save_theme_to_file(&self, theme: &Theme, file_path: impl AsRef<Path>) -> ThemeResult<()> {
        // 验证主题
        theme.validate()?;
    ...
    pub fn load_themes_from_directory(&self, dir: impl AsRef<Path>) -> ThemeResult<usize> {
        ...
        if !dir_path.exists() {
            return Err(ThemeError::IoError(format!("主题目录不存在: {:?}", dir_path)));
        }
        ...
        for entry in entries {
            let entry = entry
                .map_err(|e| ThemeError::IoError(format!("读取目录项失败: {}", e)))?;
...
impl ThemeManager {
    /// 应用主题到样式（便捷方法）
    pub fn apply_current_theme(&self, component_type: &crate::ComponentType, base_style: vizuara_core::Style) -> vizuara_core::Style {
        let current = self.current_theme.read().unwrap();
        current.apply_to_style(component_type, base_style)
    }

    /// 获取当前主题的组件颜色
    pub fn get_current_primary_color(&self, component_type: &crate::ComponentType) -> vizuara_core::Color {
        let current = self.current_theme.read().unwrap();
...
mod tests {
    ...
    fn test_register_custom_theme() {
        ...
        valid_theme.set_global(crate::ThemeProperty::PrimaryColor, crate::ThemeValue::Color(vizuara_core::Color::rgb(1.0, 0.0, 0.0)));
        valid_theme.set_global(crate::ThemeProperty::BackgroundColor, crate::ThemeValue::Color(vizuara_core::Color::rgb(1.0, 1.0, 1.0)));
        valid_theme.set_global(crate::ThemeProperty::TextColor, crate::ThemeValue::Color(vizuara_core::Color::rgb(0.0, 0.0, 0.0)));
    ...
    fn test_theme_file_operations() {
        ...
        theme.set_global(crate::ThemeProperty::PrimaryColor, crate::ThemeValue::Color(vizuara_core::Color::rgb(1.0, 0.0, 0.0)));
        theme.set_global(crate::ThemeProperty::BackgroundColor, crate::ThemeValue::Color(vizuara_core::Color::rgb(1.0, 1.0, 1.0)));
        theme.set_global(crate::ThemeProperty::TextColor, crate::ThemeValue::Color(vizuara_core::Color::rgb(0.0, 0.0, 0.0)));
    ...
    fn test_theme_stats() {
        ...
        assert_eq!(stats.total_themes, stats.preset_themes + stats.custom_themes);
